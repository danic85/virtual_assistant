...
def weather_yahoo(self, location, host, path, metric, pathtail):
    if metric == str(1):
    ...
    # Wind uses the Beaufort scale
        if self.sconfig['metric'] == str(1) and 1 == str(1):
          if wind < 1:
              gust = 'It is calm'
          if wind > 1:
              gust = 'With Light Air'
      ...
      weather_yahoo = 'Failed to connect to Yahoo Weather.  ', Argument
    print weather_yahoo
    return weather_yahoo
