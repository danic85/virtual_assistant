...
def expenses_remaining(self):
    remaining = 0.0;
    with open(self.files + '/' + self.config.get('Config','ExpensesFile'), 'rb') as csvfile:
...
def expenses_add(self):
    ...
    with open(self.files + '/' + self.config.get('Config','ExpensesFile'), 'a') as csvfile:
...
def expenses_get(self):
    f = open(self.files + '/' + self.config.get('Config','ExpensesFile'), 'r')
